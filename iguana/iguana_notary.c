...
#endif
        if ( dp->DESTHEIGHT < dp->prevDESTHEIGHT )
        {
            suppress = 1;
...
HASH_AND_STRING(dpow,updatechaintip,blockhash,symbol)
{
    ...
    if ( dp != 0 )
    {
        if ( iguana_BN_dPoWupdate(myinfo,dp) != 0 )
            sprintf(buf,MAGENTA"[%s] %s"RESET, symbol, bits256_str(str,blockhash));
        else
            sprintf(buf,YELLOW"[%s] update failed for block %s"RESET,symbol, bits256_str(str,blockhash));
    }
    else sprintf(buf,YELLOW"[%s] cannot update non-active or non dpowed coin"RESET, symbol);
    return(clonestr(buf));
}
...
char NOTARY_CURRENCIES[][65] = {
    "REVS", "SUPERNET", "DEX", "PANGEA", "JUMBLR", "BET", "CRYPTO", "HODL", "BOTS", "MGW", "MSHARK", "NINJA", "CCL", "PIRATE", "ILN", "RICK", "MORTY", "KOIN", "THC",  "GLEEC", "TOKEL", "CLC"
};
