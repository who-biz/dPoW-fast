...
int32_t dpow_is015(char *symbol)
{
    ...
    const char *dpow_015_coins[] = {"CHIPS", "SFUSD", "AYA"};
...
bits256 dpow_getbestblockhash(struct supernet_info *myinfo,struct iguana_info *coin)
{
    ...
    if ( coin->FULLNODE < 0 )
    {
        if ( coin->lastbesthashtime+1 > time(NULL) && bits256_nonz(coin->lastbesthash) != 0 )
    ...
    if ( (unspents= dpow_listunspent(myinfo,coin,coinaddr)) != 0 )
    {
        if ( (n= cJSON_GetArraySize(unspents)) > 0 )
        {
            ...
            while (haveutxo < 1)
            {
                ...
                {
                    printf("[%s] : %d out of %d loop jitem failure\n",coin->symbol,i,n);
                    i++;
                    j++;
                    continue;
                }
                ...
                {
                    printf("[%s] : %d out of %d loop not spendable\n",coin->symbol,i,n);
                    i++;
                    j++;
                    continue;
                }
